...
func (exec *FlinkExecutor) initJob(ctx context.Context, req *request.InitFlinkJob) (string, string, error) {
	...
	defer func() {
		if result != nil && !result.Status.IsFinished() {
			...
			err = qerror.ZeppelinInitFailed.Format(fmt.Sprintf("zeppelin note init failed, reason is %s", data))
		}
	}()
	udfs, err := exec.getUDFs(ctx, req.GetArgs().GetUdfs())
	if err != nil {
		return "", "", err
	}
	...
	case model.StreamJob_SQL:
		...
		if noteId, err = exec.initNote(ctx, FLINK, req.GetInstanceId(), properties); err != nil {
			return "", "", err
		}
		if result, err = exec.registerUDF(ctx, noteId, udfs); err != nil {
			return "", "", err
		}
		...
		if !hasOp {
			logger.Info().Msg("this sql does not has dml command.").Fire()
			result = &zeppelin.ParagraphResult{
				NoteId:      noteId,
				ParagraphId: "",
		...
		if paragraphId, err = exec.zeppelinClient.AddParagraph(noteId, "code", builder.String()); err != nil {
			return "", "", err
		}
		return noteId, paragraphId, err
	case model.StreamJob_Jar:
		...
		if noteId, err = exec.initNote(ctx, SHELL, req.GetInstanceId(), properties); err != nil {
			return "", "", err
		}
		...
		if jarName, jarUrl, err = exec.resourceClient.GetFileById(ctx, req.GetCode().GetJar().GetResourceId()); err != nil {
			return "", "", err
		}
		...
		if result, err = exec.zeppelinClient.Submit("sh", "", noteId, initCode); err != nil {
			return "", "", err
		}
		if paragraphId, err = exec.zeppelinClient.AddParagraph(noteId, "code", runCode); err != nil {
			return "", "", err
		}
		return noteId, paragraphId, err
	}
	return "", "", qerror.ZeppelinInitFailed.Format(fmt.Sprintf("job type note match,only support jar and sql,current type is %s", req.GetCode().GetType()))
}
...
func (exec *FlinkExecutor) submitJob(ctx context.Context, instanceId string, noteId string, paragraphId string, jobType model.StreamJob_Type) (*zeppelin.ParagraphResult, error) {
	...
	switch jobType {
	case model.StreamJob_SQL:
		if result, err = exec.zeppelinClient.SubmitParagraph(noteId, paragraphId); err != nil {
			return nil, err
		}
...
func (exec *FlinkExecutor) postHandle(ctx context.Context, instanceId string, noteId string,
	result *zeppelin.ParagraphResult) {
...
func (exec *FlinkExecutor) initNote(ctx context.Context, interceptor string, instanceId string, properties map[string]string) (string, error) {
	...
	if noteId, err = exec.zeppelinClient.CreateNote(instanceId); err != nil {
		...
		noteId, err = exec.zeppelinClient.CreateNote(instanceId)
		if err != nil {
...
func (exec *FlinkExecutor) release(ctx context.Context, instanceId string) error {
	result, err := exec.getResult(ctx, instanceId)
	if err != nil {
	...
	if len(result.NoteId) > 0 {
		_ = exec.zeppelinClient.DeleteNote(result.NoteId)
	}
